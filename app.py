...
st.markdown("_App by Shreya Mishra_")
st.markdown("""
    ...
    if extract_button and custom_file and keyword_input:
        ...
        with st.spinner("Scanning for keywords..."):
            ...
            if results:
                ...
                # Export matching lines to Excel
                output = BytesIO()
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    results_df.to_excel(writer, index=False, sheet_name='KeywordMatches')
                output.seek(0)

                st.download_button(
                    label="📥 Download Matches as Excel",
                    data=output,
                    file_name="keyword_matches.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
            else:
